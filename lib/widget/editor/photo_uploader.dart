...
class CabinPhotoGrouper extends StatefulWidget {
  ...
  CabinPhotoGrouper(this.controller);
...
class CabinPhotoGrouperState extends State<CabinPhotoGrouper> {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: GridView(
          shrinkWrap: true,
          gridDelegate:
              SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 3,childAspectRatio: 1),children:getChildern(),physics: NeverScrollableScrollPhysics(),),
    );
  }
  List<Widget> getChildern(){
    ...
    if(ret.length < 9){
      ret.add(Container(child: IconButton(icon: Icon(Icons.add), onPressed: ()async{
